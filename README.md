...
Заявка сразу подтверждается (временно, для мвп)

count - количество пассажиров, на которых требуется забронировать место

Request
```
{
    "count": 3 //по умолчанию = 1
}
```
...
```
{
    "status": 200 //или 405, если нет мест
}
```
...
Сообщение:
{
    "role":  // 1 - запрос сделал пассажир, 2 - водитель
...
Сообщение:
{
    "user": {
        "name":
        "token": //для того, чтобы потом сделать response для этого пользователя, если токен не нужен, то я уберу
    }
    "carpool_id": //айди карпула
    "peoples": //количество человек
    "role": // 1 - запрос сделал пассажир, 2 - водитель
    "status": //200 если все ок, 405 если запрос не отправился (нет мест или доступа)
}